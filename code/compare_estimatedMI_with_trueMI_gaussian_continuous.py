...
for taskid in range(len(hyperparameter_experiments)):
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for j in range(0, training_tensor.size()[0], batch_size):
            ...
            if net_name == 'Mine_Net4':
                if distribution == 'lognormal':
                    pred_xy, pred_x_y = minenet(xy=training_tensor[indices, :], x_shuffle=batch_x_shuffle, x_n_dim=1)
                elif distribution == 'gaussian':
    ...
    for type in ['training','testing']:
        ...
        if net_name == 'Mine_Net4':
            if distribution == 'lognormal':
               data_pred_xy, data_pred_x_y = minenet(xy=dataset_tensor[:, :], x_shuffle=data_x_shuffle, x_n_dim=1)
            elif distribution == 'gaussian':
               data_pred_xy, data_pred_x_y = minenet(xy=dataset_tensor[:, :], x_shuffle=data_x_shuffle, x_n_dim=gaussian_dimension)
