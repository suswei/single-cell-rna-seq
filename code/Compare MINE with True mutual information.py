...
final_dataframe.to_csv('result/Tune_Hyperparameter_For_Minenet/2019-06-04/Compare_mine_estimator_with_true_MI.csv', index=None, header=True)

for k in range(len(hyperparameter_experiments)):
    key, value = zip(*hyperparameter_experiments[k].items())
    n_hidden_z = value[0]
    n_layers_z = value[1]
    Gaussian_Dimension = value[2]
    sample_size = value[3]

    rho_2 = final_dataframe[final_dataframe.variable_dimension==Gaussian_Dimension & final_dataframe.n_hidden_z==n_hidden_z & final_dataframe.n_layers_z==n_layers_z & final_dataframe.sample_size==sample_size].loc[:,['rho']].values
    true_MI2 = final_dataframe[final_dataframe.variable_dimension==Gaussian_Dimension & final_dataframe.n_hidden_z==n_hidden_z & final_dataframe.n_layers_z==n_layers_z & final_dataframe.sample_size==sample_size].loc[:,['true_MI']].values
    estimated_training_MI2 = final_dataframe[final_dataframe.variable_dimension==Gaussian_Dimension & final_dataframe.n_hidden_z==n_hidden_z & final_dataframe.n_layers_z==n_layers_z & final_dataframe.sample_size==sample_size].loc[:,['estimated_training_MI']].values
    estimated_testing_MI2 = final_dataframe[final_dataframe.variable_dimension==Gaussian_Dimension & final_dataframe.n_hidden_z==n_hidden_z & final_dataframe.n_layers_z==n_layers_z & final_dataframe.sample_size==sample_size].loc[:,['estimated_testing_MI']].values
    ...
    fig.savefig('result/Tune_Hyperparameter_For_Minenet/2019-06-04/n_hidden_z%s_n_layers_z%s_variable_dimension%s_sample_size%s_training.png'%(n_hidden_z, n_layers_z, Gaussian_Dimension, sample_size))
    plt.close(fig)
    ...
    fig.savefig('result/Tune_Hyperparameter_For_Minenet/2019-06-04/n_hidden_z%s_n_layers_z%s_variable_dimension%s_sample_size%s_testing.png' %(n_hidden_z, n_layers_z, Gaussian_Dimension, sample_size))
    plt.close(fig)
