...
def main(dataset_name, nuisance_variable, adv_model, config_id):
    ...
    vae_MI = VAE_MI(gene_dataset.nb_genes, n_batch=gene_dataset.n_batches * use_batches, n_labels=gene_dataset.n_labels,
                    ...
                    std=std, mini_ELBO=12000, max_ELBO=18000) #mini_ELBO=15000, max_ELBO=20000
    ...
    if adv == True:
        if adv_model == 'MI':
            advnet = MINE_Net4_3(input_dim=vae_MI.n_latent + 1, n_latents=Adv_Net_architecture,
                                  ...
                                  data_loader=trainer_vae_MI_adv, drop_out = adv_drop_out, net_name = adv_model, min=-0.03, max=0.08)
        ...
        trainer_vae_MI.adv_epochs = pre_adv_epochs
        trainer_vae_MI.change_adv_epochs = adv_epochs
    ...
    if adv_model == 'MI':
        ...
        std_predicted_mutual_info = (predicted_mutual_info - (-0.03))/(0.08-(-0.03))
    elif adv_model == 'Classifier':
    ...
    std_ELBO_train = (sum(ELBO_list_train)/number_samples - 12000)/(18000 - 12000)
    ...
    for tensors in trainer_vae_MI.test_set:
        ...
        number_samples += sample_batch.shape[0]
    std_ELBO_test = (sum(ELBO_list_test) / number_samples - 12000) / (18000 - 12000)
