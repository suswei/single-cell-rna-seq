...
for taskid in range(13):
    if not os.path.exists('data/compare_estimatedMI_with_trueMI/gaussian_gaussian'):
        os.makedirs('data/compare_estimatedMI_with_trueMI/gaussian_gaussian')
    if not os.path.exists('result/ompare_estimatedMI_with_trueMI/gaussian_gaussian'):
        os.makedirs('result/compare_estimatedMI_with_trueMI/gaussian_gaussian')
        ...
        for epoch in tqdm(range(n_epochs)):
            ...
            for j in range(0, training_tensor.size()[0], batch_size):
                ...
                if Net_Name in ['Mine_Net2', 'Mine_Net3']:
                    ...
                    pred_xy, pred_x_y = minenet(xy=training_tensor[indices, :], x_shuffle=batch_x_shuffle, x_n_dim=Gaussian_Dimension)
    ...
    if Net_Name == 'Mine_Net2':
        final_dataframe.to_csv('result/compare_estimatedMI_with_trueMI/gaussian_gaussian/Taskid%s_Compare2_mine_estimator_with_true_MI.csv'%(taskid), index=None, header=True)
    elif Net_Name == 'Mine_Net3':
        final_dataframe.to_csv('result/compare_estimatedMI_with_trueMI/gaussian_gaussian/Taskid%s_Compare3_mine_estimator_with_true_MI.csv' % (taskid), index=None, header=True)
    elif Net_Name == 'Mine_Net4':
        final_dataframe.to_csv('result/compare_estimatedMI_with_trueMI/gaussian_gaussian/Taskid%s_Compare4_mine_estimator_with_true_MI.csv' % ( taskid), index=None, header=True)
