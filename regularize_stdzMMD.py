...
def main(taskid):
    ...
    os.system("python3 pareto_front_paretoMTL_main.py --regularize --use_batches --MCs 20 "
              ...
              "--MC %s --weights_total %s --n_weight %s --weight %s --num_workers %s "
              % (taskid, temp['dataset_name'], temp['confounder'], temp['n_layers_encoder'], temp['n_layers_decoder'],
                 ...
                 temp['MC'], temp['weights_total'], temp['nweight_weight']['n_weight'], temp['nweight_weight']['weight'], temp['num_workers'])
              )
